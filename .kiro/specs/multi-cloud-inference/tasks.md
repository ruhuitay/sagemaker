...
This plan implements multi-cloud MNIST inference across four layers: Alibaba Cloud deployment (ROS CDK + PAI-EAS deployer), unified inference client abstraction, tkinter desktop application with provider switching, and reliability/error handling. Tasks are ordered so that foundational abstractions (data models, interfaces) come first, followed by provider implementations, then the desktop app that wires everything together.

## Tasks

- [ ] 1. Set up project structure and core data models
  - [ ] 1.1 Create inference module package structure and data models
  ...
  - [ ] 1.2 Create Alibaba Cloud infrastructure package structure
...
- [ ] 2. Implement input validation and preprocessing
  ...
  - [ ] 2.3 Implement canvas preprocessing
  ...
  - [ ]* 2.4 Write property test for canvas preprocessing
...
- [ ] 3. Implement provider backends
  - [ ] 3.1 Implement provider backend abstract base class
  ...
  - [ ] 3.2 Implement AWS backend provider
  ...
  - [ ] 3.3 Implement Alibaba Cloud backend provider
  ...
  - [ ]* 3.4 Write property test for response normalization
  ...
  - [ ]* 3.5 Write property test for request format translation
...
- [ ] 4. Implement inference client and error handling
  - [ ] 4.1 Implement the InferenceClient class
...
- [ ] 5. Checkpoint - Core inference client
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 6. Implement configuration management
  - [ ] 6.1 Implement ConfigManager with keyring integration
  ...
  - [ ]* 6.2 Write property test for config serialization round-trip
  ...
  - [ ]* 6.3 Write property test for URL validation
  ...
  - [ ]* 6.4 Write property test for form field validation
...
- [ ] 7. Implement Alibaba Cloud deployer
  - [ ] 7.1 Implement AlicloudDeployer configuration validation
    - Create `src/alicloud_deployer.py` with `AlicloudDeployer` class
    - Implement `validate_config()`: check region against VALID_ALICLOUD_REGIONS, instance type against ALLOWED_ALICLOUD_GPU_FAMILIES prefix match, and scaling bounds (min >= 1, max <= 3, min <= max)
    - Raise `ConfigValidationError` with descriptive messages for each validation failure
    - _Requirements: 2.1, 2.2, 2.4, 2.5, 2.6, 2.7_

  - [ ]* 7.2 Write property test for instance type validation
    - **Property 2: Alicloud instance type validation**
    - Generate random instance type strings
    - Assert acceptance if and only if string starts with one of the allowed GPU families
    - **Validates: Requirements 2.2, 2.4, 2.5**

  - [ ]* 7.3 Write property test for region validation
    - **Property 3: Alicloud region validation**
    - Generate random strings
    - Assert acceptance if and only if string belongs to VALID_ALICLOUD_REGIONS set
    - **Validates: Requirements 2.1, 2.6**

  - [ ]* 7.4 Write property test for auto-scaling bounds validation
    - **Property 4: Auto-scaling bounds validation**
    - Generate random integer pairs (min_replicas, max_replicas)
    - Assert acceptance if and only if min >= 1, max <= 3, min <= max
    - **Validates: Requirements 2.3, 2.7**

  - [ ] 7.5 Implement OSS upload with retry logic
    - Implement `prepare_model_repository(model_path)`: package ONNX model into Triton model repo format (config.pbtxt + versioned model.onnx)
    - Implement `upload_to_oss(model_repo_path)`: upload to configured OSS bucket/prefix with retry (3 attempts, min 1s delay)
    - Raise `UploadError` after 3 failed attempts
    - Return OSS URI on success
    - _Requirements: 1.1, 1.6_

  - [ ]* 7.6 Write property test for OSS upload retry behavior
    - **Property 1: OSS upload retry behavior**
    - Generate sequences of upload attempts with variable failure counts (0 to 4)
    - Assert retry delay >= 1s between attempts, success on attempt N+1 when N < 3, UploadError when all 3 fail
    - **Validates: Requirements 1.6**

  - [ ] 7.7 Implement PAI-EAS service creation and lifecycle
    - Implement `create_service(oss_uri)`: create PAI-EAS Triton service with configured instance type, spot preference, and auto-scaling
    - Implement `wait_for_service(service_name, timeout=600)`: poll until running state or timeout
    - On timeout (>10 min): cleanup partial resources, raise `DeploymentError` with failure reason
    - On success: output endpoint URL and access token
    - _Requirements: 1.2, 1.4, 1.5, 1.7, 1.8_

  - [ ] 7.8 Implement service deletion and cleanup
    - Implement `delete_service(service_name, cleanup_oss=False)`: remove PAI-EAS service and compute instances
    - If `cleanup_oss=True`: delete model artifacts from OSS after service removal
    - Return `DeletionStatus` with service_id, outcome, resources_removed, and errors
    - Handle service-not-found case (return outcome="not_found")
    - Log failures with service_id/OSS key and failure reason
    - _Requirements: 3.1, 3.2, 3.3, 3.4, 3.5, 3.6_

  - [ ]* 7.9 Write property test for deletion status structure
    - **Property 11: Deletion status summary structure**
    - Generate random deletion outcomes (success, failure, not_found)
    - Assert returned DeletionStatus has non-empty service_id, valid outcome, and errors list empty when outcome is "succeeded"
    - **Validates: Requirements 3.2, 3.5**

- [ ] 8. Implement Alibaba Cloud ROS CDK infrastructure stacks
  - [ ] 8.1 Implement storage stack (OSS bucket)
  ...
  - [ ] 8.2 Implement inference stack (PAI-EAS service)
  ...
  - [ ] 8.3 Implement access stack (authentication and network)
  ...
  - [ ]* 8.4 Write unit tests for ROS CDK stack synthesis
...
- [ ] 9. Checkpoint - Infrastructure and deployer
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 10. Implement desktop application
  - [ ] 10.1 Implement provider switcher UI component
  ...
  - [ ] 10.2 Implement settings dialog
  ...
  - [ ] 10.3 Implement main application window
  ...
  - [ ] 10.4 Implement provider switching behavior
...
- [ ] 11. Implement error handling and logging in desktop app
  - [ ] 11.1 Implement error display and user actions
  ...
  - [ ] 11.2 Implement error logging to local file
  ...
  - [ ]* 11.3 Write property test for error log entry completeness
...
- ROS CDK stacks (Layer 1) and the deployer can be developed in parallel with the inference client
...
- Alibaba Cloud SDK dependencies: `alibabacloud-pai-eas-sdk`, `oss2`, `ros-cdk-core`, `ros-cdk-oss`, `ros-cdk-pai`
...
```json
{
  "waves": [
    ...
    { "id": 2, "tasks": ["2.2", "2.4", "3.2", "3.3", "7.1"] },
    { "id": 3, "tasks": ["3.4", "3.5", "4.1", "7.2", "7.3", "7.4"] },
    { "id": 4, "tasks": ["4.2", "6.1", "7.5", "8.1"] },
    { "id": 5, "tasks": ["6.2", "6.3", "6.4", "7.6", "7.7", "8.2"] },
    { "id": 6, "tasks": ["7.8", "8.3"] },
    { "id": 7, "tasks": ["7.9", "8.4", "10.1", "10.2"] },
